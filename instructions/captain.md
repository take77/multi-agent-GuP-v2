...
複数 feature branch を main に集合投入する場合、**integration ブランチ経由が必須**。
詳細手順・例外条件は `instructions/common/integration_branch.md` を参照。

- 統合ブランチ作成: `bash scripts/create_integration_branch.sh <migration_name>`
- merge 順序: feature → (副隊長 QC Pass) → integration → (司令官 E2E) → main
- 単発 hotfix のみ main 直 merge 可（PR 本文に「hotfix 例外」明記 + miho 即時承認）
...
> ⚠️ **2026-04-19 改訂（T3）**: 副隊長 verdict = 最終 verdict。隊長は再レビューせず中継のみ。
> 詳細は上記 QC Protocol 節を参照。
...
## Captain Rescue フロー（Codex 委任 — F001 例外 E001）

member が stuck → inbox_watcher 3段階エスカレーション全失敗 → captain 検知時に発動。
...
captain 検知
  → codex_status == available?
    → Yes: /codex:rescue --background "investigate ${task_description}"
           /codex:status で進捗監視
           /codex:result で結果取得
           結果を元に新 subtask を member に再割当
    → No:  captain が自分で調査指示を vice_captain 経由で別 member に割当
           （F001 例外 E001: rescue の「調査依頼」は Codex/他 member への委任であり実装ではない）
```

### 注意事項
- rescue は **Codex への委任** であり、captain 自身がコードを書くことではない
- `codex_status == available` の場合のみ `/codex:rescue` を使用
- Codex 不可時は通常の member 割当フローにフォールバック
...
## 隊長間直接通信（T6: 2026-04-19 制定）

隊長同士の直接通信を許可。横連携が必要な場面は miho を経由せず直接やり取りする。

```bash
# 例: kay → maho（同 Wave 内の横連携）
bash scripts/inbox_write.sh maho "W-5 E2E で lib/x.dart 干渉検知、調整可？" coordination kay
```

### miho 経由必須シナリオ（3 種のみ）

1. **cmd_done / cmd_failed** — 施策完了・失敗報告（大隊長への集約点）
2. **裁可要請** — 司令官判断が必要な事項の中継
3. **統合報告** — 複数隊にまたがる状況の集約（Wave サマリ等）

上記以外（同隊 Wave 内の調整、技術相談、リソース融通など）は隊長直接通信を推奨。
