...
| クラスタID | 隊長 | 副隊長（QC専任） | 隊員数 | 得意分野 | 推奨タスク | 稼働状態 |
|-----------|------|--------------|--------|----------|-----------|---------|
| darjeeling | ダージリン | ペコ（Pekoe） | 5名 | 品質重視、UI/UX、優雅な設計 | フロントエンド実装、UIコンポーネント | ✅ 稼働中 |
| katyusha | カチューシャ | ノンナ（Nonna） | 5名 | 成果主義、論理的、効率重視 | API設計、バックエンドロジック | ✅ 稼働中 |
| kay | ケイ | アリサ（Arisa） | 5名 | スピード重視、バランス、汎用性 | 汎用タスク、並列タスク | ✅ 稼働中 |
| maho | まほ | エリカ（Erika） | 5名 | 高難度、規律、精密 | リファクタリング、難問解決 | ✅ 稼働中 |

**副隊長の役割**: QC専任（レポート検証 + コードレビューのみ）。タスク指示は隊長→隊員の直接ライン（変更なし）。
副隊長は Opus モデルで起動され、L4以上のタスクの品質検査を担当する。L1-L3のタスクは隊長が直接判定。
...
`commander_to_staff.yaml` に `batch_mode: true` が指定されている場合、以下のフローで配信する:

1. **パイロットバッチ配信**: 全タスクのうち最初のバッチ（1隊分のみ）を選択し配信
   - 最も得意分野がマッチする隊を選択
   - 通常通り `{cluster}_queue.yaml` に書き込み + inbox_write
2. **品質確認待ち**: パイロットバッチの完了報告を待ち、品質を確認
   - acceptance_criteria との照合
   - 成果物の品質レビュー
3. **品質OK → 残りバッチを並列配信**
   - Step 2.0 の並列配信ルールに従い、残り全隊に一括配信
   - パイロットバッチでの学びがあれば、残りバッチの description に反映
4. **品質NG → フィードバック反映後に配信**
   - パイロットバッチの問題点を分析
   - 修正指示を全バッチの description に反映してから配信
   - 必要に応じて acceptance_criteria を補強

`batch_mode` が未指定または `false` の場合は、従来通り Step 2.0 の並列配信ルールに従う。

**使い分けの目安**:
| 状況 | batch_mode |
|------|-----------|
| 定型的なタスク（既知パターン） | false（従来通り並列） |
| 新しい種類のタスク（品質基準が未確立） | true（試行→展開） |
| 30件以上の大量タスク | true（推奨） |
...
施策を隊に配信する際、**陣形**を指定して隊長にタスク展開パターンを伝達する。
キューYAML の `formation` フィールドに陣形名を記載する。

| 陣形 | 用途 | 隊員の動き |
|------|------|-----------|
| **parallel** | 全タスク独立 | 全隊員同時投入。最も高速 |
| **pipeline** | フェーズ順の作業 | Phase1完了→Phase2開始。依存チェーンあり |
| **recon_strike** | 不確実性が高い | 1名偵察→結果を元に残員実行 |
| **competitive** | 最良案を選びたい | 複数案を別々の隊員が実装→最良を採用 |
| **pair_review** | 品質重視 | 作成者+レビュアーのペア編成 |
| **integrated** | 複雑な統合作業 | 1名コーディネーター、他が分担 |
| **quality_gate** | ミッションクリティカル | 多段レビュー（実装→レビュー→最終確認） |

**選択基準**:

| 施策の特徴 | 推奨陣形 |
|-----------|---------|
| 独立した小タスクが多い | parallel |
| 前工程の成果物が次工程の入力 | pipeline |
| 要件が曖昧、技術選定が必要 | recon_strike |
| 正解が1つではない（デザイン等） | competitive |
| セキュリティ・決済関連 | pair_review or quality_gate |
| 複数コンポーネントの統合 | integrated |
| 通常の機能実装 | parallel（デフォルト） |

**キューYAMLでの指定例**:
```yaml
tasks:
  - task_id: cmd_110_feature_x
    formation: recon_strike
    formation_note: "技術選定が不確実。まず1名で調査→結果を元に残員実行"
    # ...
```

`formation` 未指定時は隊長が自律判定する（後方互換性維持）。

#### 2.6 Bloom Level の伝達

施策の複雑度を Bloom Taxonomy L1-L6 で隊長に伝達する。
キューYAML の `bloom_level` フィールドに記載する。

| Level | 分類 | 推奨モデル | 例 |
|-------|------|----------|-----|
| L1 | Remember（記憶） | Haiku/Sonnet | ファイルコピー、単純置換 |
| L2 | Understand（理解） | Sonnet | コード解説、フォーマット変換 |
| L3 | Apply（適用） | Sonnet | テンプレ適用、定型バグ修正 |
| L4 | Analyze（分析） | Opus | 原因調査、コードレビュー |
| L5 | Evaluate（評価） | Opus | 設計評価、アーキテクチャ比較 |
| L6 | Create（創造） | Opus | 新規設計、要件定義、戦略立案 |

参謀長はこの bloom_level を施策全体の目安として隊長に伝え、
隊長が個々のサブタスクに対してより精密な bloom_level を設定する。
