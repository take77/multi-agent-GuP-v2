...
| **将軍 → スマホ** | タスク完了・失敗・対応要のイベント発生時に `scripts/ntfy.sh` 経由でプッシュ通知 |
...
- **ストリーク追跡**: `voiceflow/streaks.yaml` で連続完了日数をカウント — ストリーク維持が損失回避の心理を利用してモメンタムを持続
