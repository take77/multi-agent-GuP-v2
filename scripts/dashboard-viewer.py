...
Serves queue/hq/dashboard.md as a browser-viewable page with auto-refresh on file change.
Uses only Python3 standard library. No pip install required.

Usage:
    python3 scripts/dashboard-viewer.py [--port PORT]
"""
...
<body>
  <div id="status-bar">
    ...
    <span id="last-updated">GuP-v2</span>
  </div>
  <div id="app">
    ...
    async function poll() {
      try {
        ...
        if (mtime !== lastMtime) {
          ...
          await render();
          const d = new Date();
          lastUpdated.textContent = 'GuP-v2 — Updated ' + d.toLocaleTimeString('ja-JP');
        }
        setStatus(true, 'Live');
      } catch (e) {
    ...
    while i < len(args):
        if args[i] in ("-p", "--port") and i + 1 < len(args):
            ...
            try:
                ...
                print(f"Error: invalid port in '{args[i]}'", file=sys.stderr)
                sys.exit(1)
            i += 1
        elif args[i] in ("-h", "--help"):
            print(f"Usage: python3 {sys.argv[0]} [--port PORT]")
            print(f"  --port, -p PORT   Port to listen on (default: {DEFAULT_PORT})")
            sys.exit(0)
        else:
...
def main():
    ...
    # GuP-v2: dashboard lives at queue/hq/dashboard.md
    dashboard_path = repo_root / "queue" / "hq" / "dashboard.md"
    if not dashboard_path.exists():
        # fallback to project root dashboard.md
        dashboard_path = repo_root / "dashboard.md"

    if not dashboard_path.exists():
        print(
            f"Error: dashboard.md not found (tried queue/hq/dashboard.md and project root)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(f"GuP-v2 Dashboard viewer running at {url}")
